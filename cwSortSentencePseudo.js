...
function sort(sentence) {
    let lowerCase = [];
    let upperCase = [];
    sentence = sentence.replace(/[^\w\s]/g, '').split(' ').forEach(word => {
        word[0].toUpperCase() === word[0] ? upperCase.push(word) : lowerCase.push(word);
    });
    lowerCase = lowerCase.sort().join(' ');
    upperCase = upperCase.sort().reverse().join(' ');
